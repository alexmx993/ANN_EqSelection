...
)
{
  ...
  if (parameters$filename == "ANN_portfolio.R") {
    ...
    first_date <- as.Date(parameters$estimation_date) - 
      (neuralnet$parameters$estimation_lb + (month * 12 * 7) + neuralnet$parameters$est_range)
  } else {
    month      <- parameters$month
    first_date <- as.Date(parameters$estimation_date) - 
      (parameters$estimation_lb + (month * 12 * 7) + parameters$est_range)
  }
  
  data_out <- BatchGetSymbols(
    thresh.bad.data = 0.01,
    tickers         = ticker,
    first.date      = first_date,
    last.date       = as.Date(parameters$estimation_date),
    freq.data       = "daily",
    cache.folder    = "cache_folder"
  )
  
  data_wide <- reshape.wide(data_out$df.tickers)
  index     <- data_wide$price.adjusted[, 1]
  ...
  data_wide[["ret.closing.prices"]]  <- NULL
  data_wide[["ret.adjusted.prices"]] <- NULL
  ...
  fx_wide        <- reshape.wide(fx_out$df.tickers)
  fx_index       <- fx_wide$price.adjusted[, 1]
  fx_final_index <- as.Date(fx_index) %in% as.Date(index)
  ...
  fx_rates <- as.numeric(fx_wide$price.adjusted)

  for (i in names(data_wide)[!names(data_wide) %in% "volume"]) {
    data_wide[[i]][, 1] <- as.numeric(data_wide[[i]][, 1]) / fx_rates
  }
  
  data_wide$ret.adjusted.prices <- apply(data_wide$price.adjusted, 2, Delt)
  ...
  data_wide$tickers  <- names(data_wide$price.adjusted)
...
)
{
  ...
  data_wide   <- reshape.wide(data_out$df.tickers)
  or_index    <- data_wide$price.adjusted[, 1]
  final_index <- as.Date(or_index) %in% as.Date(benchmark$index)
  
  # Outlier detection
  alpha <- parameters$alpha
  outl  <- t(apply(data_wide$ret.adjusted.prices[, -1], 1, FUN = function(x) {
    x < quantile(x, probs = (alpha/2), na.rm = T) | x > quantile(x, probs = 1 - (alpha/2), na.rm = T)
  }))
  
  for (i in names(data_wide)) {
    data_wide[[i]][, -1] <- replace(data_wide[[i]][, -1], outl, NA)
  ...
  symbols <- names(data_wide$price.adjusted)
  ...
  fx_wide        <- reshape.wide(fx_out$df.tickers)
  fx_index       <- fx_wide$price.adjusted[, 1]
  fx_final_index <- as.Date(fx_index) %in% as.Date(benchmark$index)
  ...
  fx_rates        <- as.data.frame(fx_wide$price.adjusted)
  ...
  data_wide$fundamentals <- fundamental.ratios(prices = data_wide$price.adjusted, metadata = metadata)
